...
const UpdateDonationRequest = () => {
    ...
    const onSubmit = async (data) => {
        try {
            await api.put(`/donation-requests/${id}`, data);
            await Swal.fire({
                icon: 'success',
                title: 'Updated!',
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2 mt-4">
                    <button type="submit" className="bg-red-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-red-700 transition duration-300">Update Request</button>
                </div>
            </form>
        </div>
    );
